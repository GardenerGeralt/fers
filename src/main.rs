...
fn main() {
    ...
    let aluminium = material::Material {poisson_ratio, elastic_modulus};
    ...
    let nodes = (0..node_coordinates.len()).map(
        |node_nr| node::Node::new(
    ...
    let beam_sections = beam_areas.clone().iter().map(
        |area| section::Section {section_area: *area, material: &aluminium}
    ).collect();

    let my_model = model::Model::new(
        nodes, connectivity_matrix, beam_sections, 
